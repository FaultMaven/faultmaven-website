    ...
    <header className="sticky top-0 z-50 bg-slate-50/95 dark:bg-slate-900/80 backdrop-blur-sm border-b border-slate-200 dark:border-slate-800">
      <div className="max-w-6xl mx-auto px-6 py-2 flex justify-between items-center">
        ...
        <nav className="hidden md:flex space-x-4 text-base font-medium items-center">
          <Link href={`${process.env.NEXT_PUBLIC_DASHBOARD_URL || 'https://app.faultmaven.ai'}/login?source=website`} className="nav-underline text-blue-600 dark:text-blue-500 transition-colors duration-200">Sign In</Link>
          <Link
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
          <div className="px-6 py-4 space-y-4">
            ...
            <Link href={`${process.env.NEXT_PUBLIC_DASHBOARD_URL || 'https://app.faultmaven.ai'}/login?source=website`} className="block text-blue-600 dark:text-blue-500 hover:underline transition-colors duration-200">Sign In</Link>
            <Link
              href="https://github.com/FaultMaven/faultmaven#quick-start"
